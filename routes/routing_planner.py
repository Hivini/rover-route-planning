...
class RoverManager(object):
    ...
    def findRoute(self, sx: int, sy: int, ex: int, ey: int,
                  search_algorithm: SearchAlgorithm = SearchAlgorithm.ASTAR,
                  useNavMatrix=False):
        ...
        if useNavMatrix:
            route_finder = rf.RouteFinder(self.map_image.data, self.map_image.rows_n,
                                          self.map_image.columns_n, start_point, end_point,
                                          MAX_MOVE_HEIGHT, invalid_height=-1,
                                          navMatrix=self.map_image.regionClassifier.navMatrix)
        else:
            route_finder = rf.RouteFinder(self.map_image.data, self.map_image.rows_n,
                                          self.map_image.columns_n, start_point, end_point,
                                          MAX_MOVE_HEIGHT, invalid_height=-1)
